...
% uncomment this for automatic threshold selection
% numCameras = 18;
% thr=[];
% if loadImages
...
%     for n=1:numCameras
%         % Images
%         img = imread(sprintf('../data/david_%02d.jpg',n-1));
%         % convert to grayscale
%         gimg= rgb2gray(img);
%         t = select_thresh(gimg);
%         thr=[thr t];
%         
%     end
% end
...
silhouetteThreshold = 100; % use th_mean otherwise
%% 
...
bbox = [0.25 -0.1 -1.8; 2.1 1.1 2.5]; % [minX minY minZ; maxX maxY maxZ];
volumeX = 64;
volumeY = 64;
volumeZ = 128;
...
if computeVisualHull
    ...
    for n=1:numCameras
        ...
        for x=1:volumeX
            for y=1:volumeY
                for z=1:volumeZ
                    XYZ = T*[x-0.5,y-0.5,z-0.5,1]';
                    xy = Ps_n*XYZ;
                    xy = xy/xy(end);
                    if sils_n(floor(xy(2)),floor(xy(1))) == 1
